...
void Rabbit::move()
{
    ...
    if(m_move){
        //attempt move in random direction
        int rand = randInt(NORTH, WEST);
        if(attemptMove(*m_arena, rand, m_row, m_col))
            //check whether moved spot has a poisoned carrot
            if(m_arena->getCellStatus(m_row, m_col) == HAS_POISON){
                //eat carrot
                m_poison++;
                m_arena->setCellStatus(m_row, m_col, EMPTY);
                //rabbit is dead if consumes 2 poisoned carrots
                if(m_poison == 2){
                    m_dead = true;
                }
            }
...
int Arena::numberOfRabbitsAt(int r, int c) const
{
    ...
    int count = 0;
    for(int i = 0; i < m_nRabbits; i++){
        Rabbit* rab = m_rabbits[i];
        if(rab->row() == r && rab->col() == c)
            count++;
    }
    return count;
}
...
void Arena::display(string msg) const
{
    ...
    for(r = 1; r <= rows(); r++){
        for(c = 1; c <= cols(); c++){
            int rabbits = numberOfRabbitsAt(r, c);
            if(rabbits != 0){
                displayGrid[r-1][c-1] = (rabbits == 1 ? 'R' : '0' + rabbits);
                if(rabbits > 9) displayGrid[r-1][c-1] = '9';
            }
        }
    }
...
void Arena::moveRabbits()
{
    ...
    for(int i = 0; i < m_nRabbits; i++){
        m_rabbits[i]->move();
        if(m_rabbits[i]->row() == player()->row() && m_rabbits[i]->col() == player()->col()){
            player()->setDead();
        }
...
void Game::play()
{
    ...
    while ( ! player->isDead()  &&  m_arena->rabbitCount() > 0)
    {
        ...
        m_arena->display(msg);
        //if player voluntarily moved to a rabbit position
        if (player->isDead())
            break;
        //shift rabbits randomly
        m_arena->moveRabbits();
        m_arena->display(msg);
    }
    if (player->isDead())
...
//counts how many rabbits are surrounding the player at (r,c)
int countDangers(const Arena& a, int r, int c){
    int count = 0;
...
bool recommendMove(const Arena& a, int r, int c, int& bestDir)
{
    int dangers = countDangers(a, r, c);
    if(dangers == 0){
        return false;
    }
    ...
    for(int i = 0; i < NUMDIRS; i++){
        ...
        int col = c;
        attemptMove(a, i, row, col);
        if(a.numberOfRabbitsAt(row,col) != 0 || (row == r && col == c)) continue;
        if(countDangers(a,row,col) < minDanger){
            minDanger = countDangers(a,row,col);
            bestDir = i;
        }
    }
